...
import logging
# Importa el módulo os para interactuar con el sistema operativo, específicamente para acceder a variables de entorno.
import os
# Importa el módulo json para trabajar con datos en formato JSON, necesario para parsear las credenciales de Firebase.
import json
...
def initialize_firestore():
    ...
    if not firebase_credentials_json:
        ...
        logging.error(
            "❌ La variable de entorno 'FIREBASE_CREDENTIALS_JSON' no está configurada. No se puede inicializar Firestore.")
        return None
    ...
    try:
        ...
        logging.error(
            f"❌ Error al inicializar Firebase o Firestore: {e}", exc_info=True)
...
def get_firestore_db():
    ...
    # Devuelve la instancia (ya sea la recién inicializada o una existente).
    return db
