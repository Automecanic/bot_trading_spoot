...
import os
# Importa el nuevo módulo para Firestore, que permite la interacción con la base de datos Firestore.
...
def load_parameters():
    ...
    if db:  # Si la conexión a Firestore es exitosa (db no es None).
        try:
            ...
            doc_ref = db.collection(FIRESTORE_CONFIG_COLLECTION_PATH).document(
                FIRESTORE_CONFIG_DOC_ID)
            ...
            if doc.exists:  # Si el documento existe en Firestore.
                ...
                logging.info(
                    f"✅ Parámetros cargados desde Firestore: {FIRESTORE_CONFIG_COLLECTION_PATH}/{FIRESTORE_CONFIG_DOC_ID}. Beneficio cargado: {params.get('TOTAL_BENEFICIO_ACUMULADO', 0.0):.2f} USDT")
                ...
                logging.warning(
                    f"⚠️ Documento de configuración no encontrado en Firestore: {FIRESTORE_CONFIG_COLLECTION_PATH}/{FIRESTORE_CONFIG_DOC_ID}. Intentando cargar desde archivo local.")
        except Exception as e:
            # Si ocurre cualquier error durante la carga desde Firestore, registra el error y el fallback.
            logging.error(
                f"❌ Error al cargar parámetros desde Firestore: {e}", exc_info=True)
            logging.warning(
                "⚠️ Fallback: Intentando cargar desde archivo local.")

    # Fallback a archivo local: Si Firestore no estaba disponible o falló.
    # Verifica si el archivo de configuración local existe.
    if os.path.exists(CONFIG_FILE):
        try:
            ...
            logging.info(
                f"✅ Parámetros cargados desde {CONFIG_FILE}. Beneficio cargado: {params.get('TOTAL_BENEFICIO_ACUMULADO', 0.0):.2f} USDT")
            return params  # Devuelve los parámetros cargados.
            ...
            logging.error(
                f"❌ Error al cargar parámetros desde {CONFIG_FILE}: {e}")

    # Si no se pudo cargar desde Firestore ni desde el archivo local, se cargan los parámetros por defecto.
    logging.warning(
        "⚠️ No se encontró archivo de configuración local o hubo un error. Cargando parámetros por defecto.")
    # Definición de los parámetros por defecto del bot.
    default_params = {
        # Intervalo de tiempo en segundos entre cada ciclo de trading principal (15 minutos).
        "INTERVALO": 900,
        # Porcentaje del capital total a arriesgar por operación (1%).
        "RIESGO_POR_OPERACION_PORCENTAJE": 0.01,
        # CAMBIO: Porcentaje de ganancia para cerrar una posición (Take Profit) (5%).
        "TAKE_PROFIT_PORCENTAJE": 0.05,
        # CAMBIO: Porcentaje de pérdida para cerrar una posición (Stop Loss fijo) (3%).
        "STOP_LOSS_PORCENTAJE": 0.03,
        # CAMBIO: Porcentaje para activar el Trailing Stop Loss (2.5%).
        "TRAILING_STOP_PORCENTAJE": 0.025,
        ...
        # Beneficio total acumulado por el bot desde su inicio (inicialmente 0.0).
        "TOTAL_BENEFICIO_ACUMULADO": 0.0,
        # Porcentaje de ganancia para mover el Stop Loss a Breakeven (0.5%).
        "BREAKEVEN_PORCENTAJE": 0.005,
        # Período para análisis de rangos (en velas)
        "RANGO_PERIODO_ANALISIS": 20,
        "RANGO_UMBRAL_ATR": 0.015,     # Umbral de ATR para considerar mercado en rango
        "RANGO_RSI_SOBREVENTA": 30,    # RSI mínimo para comprar en rango
        "RANGO_RSI_SOBRECOMPRA": 70,   # RSI máximo para vender en rango
        "RANGO_OPERAR": True           # Activar/desactivar operaciones en rango
...
def save_parameters(params):
    ...
    if db:  # Si la conexión a Firestore es exitosa.
        try:
            ...
            logging.info(
                f"DEBUG: Guardando parámetros en Firestore. Beneficio a guardar: {params.get('TOTAL_BENEFICIO_ACUMULADO', 0.0):.2f} USDT")
            # Obtiene una referencia al documento de configuración en Firestore.
            doc_ref = db.collection(FIRESTORE_CONFIG_COLLECTION_PATH).document(
                FIRESTORE_CONFIG_DOC_ID)
            # Guarda el diccionario de parámetros en el documento de Firestore.
            doc_ref.set(params)
            logging.info(
                f"✅ Parámetros guardados en Firestore: {FIRESTORE_CONFIG_COLLECTION_PATH}/{FIRESTORE_CONFIG_DOC_ID}")
            ...
            logging.error(
                f"❌ Error al guardar parámetros en Firestore: {e}", exc_info=True)
            logging.warning(
                "⚠️ Fallback: Intentando guardar en archivo local.")

    # Fallback a archivo local: Si Firestore no estaba disponible o falló.
    try:
        with open(CONFIG_FILE, 'w') as f:  # Abre el archivo en modo escritura.
            # Guarda los parámetros en formato JSON con indentación para legibilidad.
        ...
        logging.error(
            f"❌ Error inesperado al guardar parámetros en {CONFIG_FILE}: {e}")
        return False  # Indica que el guardado falló.
